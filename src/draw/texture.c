...
static struct wlr_buffer *texture_make_ring(struct wlr_buffer *canvas, int width, int height, int border, double corners) {
...
// gradient-specific functions (key empty check, key equal check, key copy, key destroy)
// to implement another renderer, these must be provided to the texture renderer using TextureOps

static bool gradient_key_empty(const BorderTextureKey *key) {
    return key->gradient.stopcount <= 0;
}

static bool gradient_key_equal(const BorderTextureKey *a, const BorderTextureKey *b) {
    if (a->gradient.stopcount != b->gradient.stopcount)
        return false;
    
    for (int index = 0; index < a->gradient.stopcount; index++) {
        if (memcmp(a->gradient.stops[index].color, b->gradient.stops[index].color, sizeof(float) * 4) != 0)
            return false;
...
static bool gradient_key_copy(const BorderTextureKey *source, BorderTextureKey *destination) {
    ...
    destination->gradient.stops = malloc((size_t)source->gradient.stopcount *sizeof(GradientStop));
    if (destination->gradient.stops == NULL)
        return false;
    memcpy(destination->gradient.stops, source->gradient.stops, (size_t)source->gradient.stopcount * sizeof(GradientStop));
...
static struct wlr_buffer *texture_render_gradient(const BorderTextureKey *key, Client *target) {
    (void)target;
...
bool texture_key_copy(const BorderTextureKey *source, BorderTextureKey *destination) {
...
static bool texture_cache_store(const BorderTextureKey *key, struct wlr_buffer *canvas) {
...
struct wlr_buffer *texture_cache_get(const BorderTextureKey *key, Client *target, bool *from_cache) {
...
static void texture_use_count(void) {
    ...
    wl_list_for_each(client, &clients, link) {
        for (size_t index = 0; index < texture_cache_count; index++) {
        if (texture_key_equal(&texture_cache[index].key, &client->active_texture))
            texture_cache[index].use_count++;
        if (texture_key_equal(&texture_cache[index].key, &client->inactive_texture))
            texture_cache[index].use_count++;
        }
    }
}
