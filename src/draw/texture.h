...
struct TextureOps {
	...
	bool (*key_copy)(const BorderTextureKey *source, BorderTextureKey *destination);
	void (*key_destroy)( BorderTextureKey *key);
	bool bypass_cache;
	struct wlr_buffer *(*render)(const BorderTextureKey *key, struct Client *target);
	
};
void texture_style_register(TextureStyle style, struct TextureOps ops);
struct wlr_buffer *texture_cache_get(const BorderTextureKey *key, struct Client *target, bool *from_cache);
